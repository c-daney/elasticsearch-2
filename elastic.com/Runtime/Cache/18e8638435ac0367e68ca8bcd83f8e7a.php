...
<div class="container">
	...
	<p style="border-bottom: 1px solid #eee;padding-bottom:12px; margin-bottom:12px;padding-left:12px;"><a href="<?php echo U('Index/genre');?>" title="">genre电台</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="<?php echo U('Index/mood');?>" title="">mood电台</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="<?php echo U('Index/style');?>" title="" style="font-weight:700;font-size:18px;">style电台</a>&nbsp;&nbsp;&nbsp;&nbsp;<a href="<?php echo U('Index/theme');?>" title="">theme电台</a></p>
	<form metod="get" action="<?php echo U('Index/style');?>" name="form2" id="getData" class="form-inline" style="margin-bottom:20px;">
		...
		<select name="status" class="form-control" onChange="document.getElementById('getData').submit();" style="width:200px;">
			<option value="1" <?php if(in_array(($_GET['status']), explode(',',"1"))): ?>selected="selected"<?php endif; ?>>查看所有有效style电台</option>
			<option value="0" <?php if(in_array(($_GET['status']), explode(',',"0"))): ?>selected="selected"<?php endif; ?>>查看所有无style效电台</option>
		</select>
	...
	<table class="table table-hover table-bordered">
		...
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$va): $mod = ($i % 2 );++$i;?><tr><td><input type="checkbox" name="id[]" value="<?php echo ($va["typeid"]); ?>" class="checkone" /></td><td><?php echo ($va["typeid"]); ?></td><td><?php echo ($va["title"]); ?></td><td><a href="<?php echo ($va["coversrc"]); ?>" title="<?php echo ($va["coversrc"]); ?>" target="_blank"><?php echo (msubstr($va["coversrc"],0,35)); ?></a></td><td><?php echo ($va["status"]); ?></td><td><?php if(!isset($_GET['status']) || $_GET['status'] == 1): ?><a href="<?php echo U('Index/delStyleOne',array('id'=>$va['typeid']));?>" onclick="return confirm('确认要执行删除操作吗？继续，该条数据状态将标为0')">删除</a><?php else: ?><a href="<?php echo U('Index/recoverStyleOne',array('id'=>$va['typeid']));?>" onclick="return confirm('确认要执行恢复操作吗？继续，该条数据状态将标为1')">恢复</a><?php endif; ?> &nbsp;&nbsp;<a href="<?php echo U('Index/updateStyleOne',array('id'=>$va['typeid']));?>" title="">更新</a></td></tr><?php endforeach; endif; else: echo "" ;endif; ?>
		<tr><td><input type="checkbox" class="checkAll" /></td><td colspan="5"><?php if(!isset($_GET['status']) || $_GET['status'] == 1): ?><input type="button" class="btn btn-info btn-xs" value="删除所选" onclick="if (confirm('确定要删除选中的style电台吗？确认，所选电台状态将标为0')) { btnSubmit('form[name=form]', 'index.php?m=Index&a=delStyle');} else { return false;}"><?php else: ?><input type="button" class="btn btn-info btn-xs" value="恢复所选" onclick="if(confirm('确定要恢复状态为无效的style电台吗？确认，所选电台状态将标为1')){ btnSubmit('form[name=form]','index.php?m=Index&a=recoverStyle');}"><?php endif; ?></td></tr>
